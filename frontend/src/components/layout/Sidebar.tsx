...
export default function Sidebar({ isCollapsed, onToggle }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div
        className={`
          fixed top-0 left-0 h-screen bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 
          flex flex-col transition-all duration-300 ease-in-out z-40
          ${isCollapsed ? "w-20" : "w-64"}
          ${isCollapsed ? "-translate-x-full lg:translate-x-0" : "translate-x-0"}
        `}
      >
        {/* Logo Section - Condensed when collapsed */}
        <div
          onClick={() => navigate("/")}
        ...
        >
          <div className={`flex items-center ${isCollapsed ? "justify-center" : "gap-3"}`}>
            <div className="w-9 h-9 bg-slate-900 dark:bg-slate-800 rounded-xl flex items-center justify-center shadow-sm flex-shrink-0">
              <span className="text-white dark:text-slate-200 font-bold text-sm tracking-wider">
                TS
              </span>
            </div>
            {!isCollapsed && (
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 custom-scrollbar">
              ...
              <div className="space-y-1 mt-2">
                {projectItems.slice(0, 3).map((project) => (
                  <NavLink
                    ...
                    to={project.path}
                    className={({ isActive }) => `
                      flex justify-center px-2 py-2 rounded-lg transition-colors
          ...
          {/* Priority Tasks - Hide when collapsed */}
          {!isCollapsed && (
            <div className="mb-6">
              <p className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest px-3 mb-3">
                Up Next
              ...
              <div className="space-y-1">
                ...
                >
                  <div className="flex items-start gap-2">
                    <CheckCircle2
                      size={16}
                      className="text-slate-300 dark:text-slate-600 group-hover:text-emerald-500 dark:group-hover:text-emerald-400 transition-colors mt-0.5 flex-shrink-0"
                    />
                    <div>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-2">
          ...
          <button
            onClick={onToggle}
            className="hidden lg:flex w-full items-center justify-center gap-2 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-slate-500 dark:text-slate-400"
          ...
          <button
            ...
            className={`w-full flex items-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors group p-2 ${
              isCollapsed ? "justify-center" : "justify-between"
            }`}
            title={isCollapsed ? "Logout" : ""}
          >
            <div className={`flex items-center ${isCollapsed ? "justify-center" : "gap-3"}`}>
              <div className="w-8 h-8 rounded-full bg-indigo-600 dark:bg-indigo-500 flex items-center justify-center text-white text-xs font-bold shadow-sm ring-2 ring-white dark:ring-slate-800 flex-shrink-0">
                AD
              </div>
              {!isCollapsed && (
            ...
            {!isCollapsed && (
              <LogOut
                size={16}
                className="text-slate-400 dark:text-slate-500 group-hover:text-slate-900 dark:group-hover:text-slate-300 transition-colors"
              />
            )}
          </button>
        </div>
