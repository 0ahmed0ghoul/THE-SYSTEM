...
export class DashboardService {
     ...
     */
    static async getDashboardData(options: GetDashboardOptions): Promise<DashboardData> {
        const [stats, recentProjects, upcomingTasks, recentActivity, metrics, chartData] = await Promise.all([
    ...
    static async getStats(userId?: number): Promise<DashboardStats> {
        ...
        const taskStatsQuery = `
            SELECT 
                ...
                SUM(CASE WHEN status NOT IN ('done') THEN 1 ELSE 0 END) as active_tasks,
        ...
        const [projectRows] = await db.query<ProjectStatsRow[]>(projectStatsQuery, values);
    ...
    static async getMetrics(userId?: number): Promise<DashboardMetrics> {
        ...
        const [rateRows] = await db.query<CompletionRateRow[]>(completionRateQuery, values);
    ...
    static async getChartData(
        ...
        const [weeklyRows] = await db.query<WeeklyRow[]>(weeklyQuery, [days, ...(userId ? [userId] : [])]);
        const [trendRows] = await db.query<TrendRow[]>(trendQuery, [days, ...(userId ? [userId] : [])]);
        ...
        const priorityDistribution = priorityRows.map((row: PriorityRow) => ({
            name: row.priority === 'urgent' ? 'S-Rank' :
                  row.priority === 'high' ? 'A-Rank' :
                  row.priority === 'medium' ? 'B-Rank' : 'C-Rank',
            value: row.count,
            color: priorityColorMap[row.priority],
            rank: row.priority === 'urgent' ? 'S' :
                  row.priority === 'high' ? 'A' :
                  row.priority === 'medium' ? 'B' : 'C',
            order: row.priority === 'urgent' ? 1 :
                   row.priority === 'high' ? 2 :
                   row.priority === 'medium' ? 3 : 4
        }));
    ...
    static async getPriorityDistribution(
        ...
        const priorityMap: Record<string, { count: number; name: string; color: string; rank: string; order: number }> = {
            urgent: { count: 0, name: 'S-Rank', color: '#ff6b6b', rank: 'S', order: 1 },
            high: { count: 0, name: 'A-Rank', color: '#ffb347', rank: 'A', order: 2 },
            medium: { count: 0, name: 'B-Rank', color: '#4fc3f7', rank: 'B', order: 3 },
            low: { count: 0, name: 'C-Rank', color: '#4fe6a0', rank: 'C', order: 4 }
        };

        const dateFilter = timeframe !== 'all' 
            ? `AND created_at >= DATE_SUB(CURDATE(), INTERVAL ${timeframe === 'week' ? 7 : timeframe === 'month' ? 30 : 365} DAY)`
            : '';
    ...
    static async getPriorityDistributionWithDetails(
        ...
        return {
            ...
            lowestPriority: [...distribution].reverse().find(d => d.value > 0)?.name || 'None',
            averagePriorityScore: this.calculateAveragePriorityScore(distribution, total)
        };
    }

    private static calculateAveragePriorityScore(distribution: PriorityDistributionItem[], total: number): number {
    ...
    static async comparePriorityDistribution(
        ...
        const trends = distribution1.map(item => {
            ...
            const previous = distribution2.find(d => d.name === item.name)?.value || 0;
        ...
        const totalPercentageChange = total2 > 0 ? (totalChange / total2) * 100 : (total1 > 0 ? 100 : 0);
    ...
    static async getPriorityDistributionTrends(
        ...
        const timeline: any[] = [];
        const priorityTrendsMap = new Map<string, Array<{ date: Date; value: number }>>();
        ...
        for (let i = intervals - 1; i >= 0; i--) {
            ...
            const distribution = await this.getPriorityDistributionForDateRange(userId, startDate, endDate);
            ...
            priorities.forEach(priority => {
                ...
                if (!distribution.find((d: PriorityDistributionItem) => d.name === priority)) {
        ...
        const priorityTrends = Array.from(priorityTrendsMap.entries()).map(([priority, values]) => {
        ...
        const averageGrowth = priorityTrends.reduce((sum, current) => sum + current.trend, 0) / priorityTrends.length;
        
        const mostStable = priorityTrends.reduce((most, current) => {
            const variance = this.calculateVariance(current.values.map(v => v.value));
            const mostVariance = this.calculateVariance(most.values.map(v => v.value));
